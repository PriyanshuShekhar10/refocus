...
## Phase 4: DRY Code & Logic Consolidation
During a comprehensive deep-search audit, two major areas of duplicated and conflicting logic were identified and resolved to prevent future technical debt and bugs.
1. **Unified Pub/Sub Broadcasting**: Across 15+ different API routes, the codebase manually called both local Server-Sent Events (SSE) publishers and Ably remote publishers side-by-side in `Promise.all` arrays. This brittle pattern was centralized into a single `broadcastEvent` function inside a new `lib/broadcaster.ts` module, and all routes were refactored to use it, drastically reducing boilerplate and centralizing pub/sub logic.
2. **Consolidated Session Request Creation**: The exact same 60 lines of complex session request validation (checking schedule boundaries, max future days, duration options) and database insertion logic were duplicated across two separate API endpoints (`/api/session-requests` and `/api/chat/[friendId]`). The former endpoint failed to generate a chat message, causing desyncs in chat history. This was fully refactored by creating a shared `createSessionRequest` helper in `lib/sessionRequests.ts` and wiring both endpoints to use it, guaranteeing identical and correct behavior.

## Phase 5: Deep Scalability Audit (Cron & Reminders)
During the fifth phase, a severe bottleneck was uncovered in the background scheduled jobs (`session-reminders` running every 5 mins). 
1. **O(Total Users) Memory Leak**: The cron job's underlying query logic (`findReminderRecipients`) performed an unbounded scan to load *all* registered users who had email reminders enabled, and then fired sequential database queries (`findUserSessionsInRange`) to determine if each user had a session. This triggered a catastrophic N+1 query avalanche.
2. **Cardinality Inversion Fix**: The job was completely redesigned in `lib/sessionReminders.ts` and `lib/sessionReminderJobs.ts` to flip the cardinality. It now queries `sessions` starting in the target time window (O(Active Sessions)), extracts the participants, and bulk loads their user profiles via a single `$in` query. This completely eliminates the bottleneck and ensures the jobs execute in milliseconds regardless of overall user growth.
